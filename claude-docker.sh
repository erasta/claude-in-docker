...
lines = [
    ...
    "# See `claude-docker.sh --help-capabilities` for the full schema reference.",
    "version: 1",
    "capabilities:",
    "  # gpu: nvidia | dri | kfd | none",
    "  #   nvidia → --gpus all; requires nvidia runtime in `docker info`",
    "  #   dri    → --device /dev/dri/renderD* (AMD/Intel VA-API)",
    "  #   kfd    → --device /dev/kfd (AMD ROCm)",
    "  #   none   → no GPU flag",
    f"  gpu: {gpu}",
    "",
    "  # network_mode: host | bridge | none",
    "  #   host   → --network host; container shares host stack (localhost = host)",
    "  #   bridge → docker default; container gets a private network",
...
# generator's commented options (above) and the parser's CAP_ALLOWED table.
capabilities_help() {
  cat <<'CAPHELP'
capabilities.conf — schema reference

version: 1                                # only supported version

capabilities:
  gpu: nvidia | dri | kfd | none          # default: none
    nvidia → --gpus all; requires nvidia runtime in `docker info`
    dri    → --device /dev/dri/renderD*  (AMD/Intel VA-API)
    kfd    → --device /dev/kfd            (AMD ROCm)
    none   → no GPU flag
  ...
  python_mode: link | copy                # default: link
    link → mount the host venv/interpreter into the container read-only
           (fast; container tracks host changes; host uid must match)
    copy → use the image's baked venv (self-contained; slower to rebuild)
...
resources:                                # OPEN-ENDED — add one entry per host
  ...
  <name>:                                 # matches [a-zA-Z_][a-zA-Z0-9_-]*
    cli: <cmd>                            # optional — asserted on host PATH at parse
    mounts:                               # list of host:container[:ro]
      - "${HOME}/some/path:/opt/name:ro"
    env:                                  # list of NAMES to forward from launcher env
      - GH_TOKEN
    env_set:                              # mapping of literal KEY: VALUE (non-secret)
      SSH_AUTH_SOCK: /ssh-agent

Notes:
- env:     NAMES only; docker resolves values from the launcher's env at start.
- env_set: literal KEY: VALUE pairs. Never write secrets to this file — put them
           behind env: (forwarded from the shell).
- mount paths accept ~ and ${VAR}; source must exist on the host at parse time.
- Unknown keys under capabilities: are rejected. Unknown top-level keys are
  rejected. Missing / unsupported `version:` is rejected.

Generate a fresh file:
  claude-docker.sh --generate-capabilities [--output PATH]
                   [--gpu=VAL] [--network-mode=VAL] [--python-mode=VAL]
                   [--resource NAME:cli=CLI,mount=H:C[:ro],env=KEY,env_set=K=V ...]
CAPHELP
}
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    --generate-capabilities)
      ...
      ;;
    --help-capabilities)
      # evolvix#952: print the schema reference (one place to see every option).
      capabilities_help
      exit 0
      ;;
    --name)
...
Capability contract (evolvix#931, reshaped by #935):
  ...
  --generate-capabilities [--output FILE] [--interactive]
                        ...
                        Every option in the generated file is preceded by a
                        commented reference line listing accepted values +
                        effect (evolvix#952).
